...
void addHolePathH(std::vector<SceneObject> &scene, glm::vec3 pos,
  ...
  scene.push_back(ShapeFactory::createCube(pos, scale, rot, turf));
  float hd = scale.z * 0.5f + 0.2f;
  addBorder(scene, pos + glm::vec3(0, 0.3f, -hd), {scale.x, 0.6f, 0.4f}, rot);
  addBorder(scene, pos + glm::vec3(0, 0.3f,  hd), {scale.x, 0.6f, 0.4f}, rot);
}
...
if (lamp)  meshInstances.push_back({lamp,  {h1X + 3.0f, 0.0f, h1Z - 3.0f}, glm::vec3(1.0f), 0.0f});
 ...
 // Hole 2
  ...
  addBorder(sceneObjects, {h4X + 4.0f, 0.5f, h4Z + 6.0f}, {0.4f, 0.6f, 4.5f},
            {0, 15, 0});
    std::vector<MeshInstance> meshInstances4;
  if (bench)    meshInstances4.push_back({bench,    {h4X + 6.0f, 0.0f, h4Z + 5.0f}, glm::vec3(1.0f), 90.0f});
  if (boulders) meshInstances4.push_back({boulders, {h4X - 6.0f, 0.0f, h4Z + 8.0f}, glm::vec3(1.0f), 0.0f});
  if (rocks)    meshInstances4.push_back({rocks,    {h4X + 6.0f, 0.0f, h4Z + 10.0f}, glm::vec3(1.0f), 60.0f});
...
std::vector<MeshInstance> meshInstances5;
            if (lamp)  meshInstances5.push_back({lamp,  {h5X - 4.0f, 0.0f, h5Z + 3.0f}, glm::vec3(1.0f), 0.0f});
if (lamp)  meshInstances5.push_back({lamp,  {h5X + 4.0f, 0.0f, h5Z + 8.0f}, glm::vec3(1.0f), 0.0f});
if (tree)  meshInstances5.push_back({tree,  {h5X + 5.0f, 0.0f, h5Z - 2.0f}, glm::vec3(1.0f), 0.0f});
if (rocks) meshInstances5.push_back({rocks, {h5X - 5.0f, 0.0f, h5Z + 6.0f}, glm::vec3(1.0f), 0.0f});
